...
The core of this project is a React-based interactive demo (`ShortestPathLab_1.jsx`) that allows users to visually step through algorithms and explore a rich narrative Timeline:
...
4. **The "Harder Sibling" (2024–2026)**: A dedicated Story tab and visual Timeline covering the recent rapid breakthroughs in handling *real negative weights* faster than Johnson's $O(mn)$, spanning from Fineman's 2024 randomized $O(mn^{8/9})$ to Quanrud & Tajkhorshid's 2026 negative-edge sparsification ($\approx O(mn^{0.72})$).
